...
AVAL_BIN_DIR="$AVAL_REMOTE/home/a90113/labs/mpi/bin"
...
run_matrix_multiplication() {
    ...
    for size in "${sizes[@]}"; do
        ...
        # scp "$MULTIPLY_MATRIX_EXE" "$AVAL_BIN_DIR"
        # if [ $? -ne 0 ]; then
        ...
        # { time mpiexec --host fct-deei-aval:2,fct-deei-linux:4 -np 6 -wd /home/a90113/labs/mpi/scripts/ ./"$MULTIPLY_MATRIX_EXE" "$input_file" "$LOG_TIMES" "$LOG_RESULTS"; } 2>>"$LOG_TIMES"
        if [ $? -ne 0 ]; then
            echo "Execution failed."
            rm -f "$MULTIPLY_MATRIX_EXE"
            exit 1
