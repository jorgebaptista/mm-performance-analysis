...
#SBATCH --job-name=hyb-dynio-float-numa-blas-2-15
...
MULTIPLY_MATRIX_SOURCE="../src/multiply_matrix_dyn_io_blas.c"
...
run_matrix_multiplication() {
    ...
    for ((power = MIN_P; power <= MAX_P; power++)); do
        ...
        /usr/bin/time -v -o "$LOG_TIMES" -a srun \
            --ntasks=$SLURM_NTASKS \
            --cpus-per-task=6 \
            --cpu-bind=cores \
            --distribution=block:block \
            ./"$MULTIPLY_MATRIX_EXE" "$input_file" "$LOG_TIMES" "$LOG_RESULTS"
