...
#SBATCH --job-name=blas-omp-float
...
module load gcc11/libs/openblas/0.3.21
...
run_matrix_multiplication() {
    ...
    for ((power = MIN_P; power <= MAX_P; power++)); do
        ...
        gcc -fopenmp -O3 -o "$MULTIPLY_MATRIX_EXE" "$MULTIPLY_MATRIX_SOURCE" -DSIZE=$size -DMAX_SIZE=$((2 ** $MAX_P)) -DMATRIX_TYPE=$MATRIX_TYPE -DNRUNS=$NRUNS -DTHREADS=$THREADS -lopenblas
        if [ $? -ne 0 ]; then
            echo "Compilation failed."
            break
        fi
...
echo "Allocated Memory (per node): $((SLURM_CPUS_ON_NODE * SLURM_MEM_PER_CPU))MB" >>"$LOG_TIMES"
